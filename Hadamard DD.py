...
amp = (0.17420911773387815+0j)#quito Pi amplitude
amp90 = 0.08609978094368127+0.0011775647827442363j #quito pi/2 amplitude
sigma = 40
num_samples = 160
beta= 0.4839087515594069
beta90=0.3698792662654741
...
y90d0=pulse.library.Drag(num_samples, amp90, sigma, beta90,name="yp/2_d0")

with pulse.build(backend,name='Custom_Not') as n_q0:
    
     pulse.play(x90d0, channel)
     pulse.shift_phase(-2.3561944902,channel)
     pulse.shift_phase(-2.3561944902,ControlChannel(1))
     pulse.play(x90d0, channel)
